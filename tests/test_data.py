...
class TestBLMData(unittest.TestCase):
    ...
    def test_iter_max(self):
        expected = [
            (
                (
                    "INJPHYS",
                    pd.to_datetime("2018-10-02 10:14:24.736000061+0200").tz_convert(
                        "Europe/Zurich"
                    ),
                ),
                [
            ...
            (
                (
                    "INJPHYS",
                    pd.to_datetime("2018-10-02 10:21:31.855000019+0200").tz_convert(
                        "Europe/Zurich"
                    ),
                ),
            ...
            (
                (
                    "INJPHYS",
                    pd.to_datetime("2018-10-02 10:40:34.857000113+0200").tz_convert(
                        "Europe/Zurich"
                    ),
                ),
            ...
            (
                (
                    "INJPHYS",
                    pd.to_datetime("2018-10-02 10:40:03.769000053+0200").tz_convert(
                        "Europe/Zurich"
                    ),
                ),
                [
    ...
    def test_get_beam_meta(self):
        out = BLM_DATA.fetch_intensity(beam=1)
        self.assertEqual(out.shape, (1686, 1))

        out = BLM_DATA.fetch_filling_scheme()
        self.assertEqual(out.iloc[0][0], "25ns_2556b_2544_2215_2332_144bpi_20injV3")

        out = BLM_DATA.fetch_number_bunches(beam=1)
        self.assertEqual(out.iloc[0][0], 2556.0)

        # out = BLM_DATA.get_beam_meta('energy')
        # self.assertEqual()

    def test_lossmap(self):
        LM = BLM_DATA.loss_map(
            datetime="2018-10-02 10:14:15.771000147+02:00",
            context={"foo": "bar"},
        ...
        self.assertEqual(LM.get_background().df["data"].mean(), 3.7012022404726227e-07)
